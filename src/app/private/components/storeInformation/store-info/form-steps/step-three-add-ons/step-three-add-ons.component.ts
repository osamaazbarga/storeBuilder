...
export class StepThreeAddOnsComponent implements OnInit {
  ...
  getPrice(addOn:any){
    if(addOn!=null)
      return addOn[this.timeFrame].price
    return null
  }
  ...
  updateAddOns() {
    ...
    let serviceCostUpdated = this.stepForm.value.service ? (this.addOnOptions[0] as any).timeFrame[this.timeFrame].addToTotal : 0;
    let storageCostUpdated = this.stepForm.value.storage ? (this.addOnOptions[1] as any).timeFrame[this.timeFrame].addToTotal : 0;
    let customizationCostUpdated = this.stepForm.value.customization ? (this.addOnOptions[2]as any).timeFrame[this.timeFrame].addToTotal : 0;
  ...
  toggleAddOn(event: any, addOn: any) {
    ...
    let timeFrame = this.addOnOptions[this.addOnOptions.findIndex(a => a.formName == addOn)].timeFrame;
    let addOnCost:any=(timeFrame as any)[this.timeFrame].addToTotal
